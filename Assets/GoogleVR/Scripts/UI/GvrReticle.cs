...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  public void OnGazeStay(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
    ...
    SetGazeTarget(intersectionPosition, isInteractive);
        ...
        if (targetObject.GetComponentInChildren<Canvas>() != null)
        {
            targetObject.GetComponentInChildren<Canvas>().enabled = true;
            targetObject.GetComponentInChildren<Slider>().value = (Time.time - gazeStartTime) / 2;
        }

        if (gazedAt != null && gazeStartTime > 0f) {
            if (Time.time - gazeStartTime > 2 && ExecuteEvents.CanHandleEvent<TimedInputHandler>(gazedAt)) {
                gazeStartTime = -1f;
  ...
  public void OnGazeExit(Camera camera, GameObject targetObject) {
    ...
    if (targetObject.GetComponentInChildren<Canvas>() != null)
    {
            targetObject.GetComponentInChildren<Canvas>().enabled = false;
            targetObject.GetComponentInChildren<Slider>().value = 0;
        }
  }
